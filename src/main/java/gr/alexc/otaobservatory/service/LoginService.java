...
public class LoginService {
    ...
    public User getUser(String email, String password) {
        ...
        userDetails.setExpirationDate(jwtUtilService.extractClaim(userDetails.getToken(), Claims::getExpiration));
    ...
    public User getCurrentSession(String token) {
        ...
        //reset token if no longer active
        if(extractedClaimExpiration.getTime() <= new Date().getTime()){
            foundUserOpt.ifPresent(foundUser -> {
                foundUser.setToken(null);
            loginRepository.save(foundUser);
                throw new ExpiredTokenException(token + " for " + foundUserOpt + "has expired");
            });
    ...
    public User invalidateCurrentSession(String token) {
        ...
        foundUserOpt.ifPresent(foundUser -> {foundUser.setToken(null);
